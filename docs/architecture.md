...
**ADR author agent.** Takes the findings and produces the content of an Architecture Decision Record: title, decision, drivers, conditions, and consequences. It works from the findings, not from the original submission, which keeps it grounded in what was actually assessed.
...
Two modules are in the repository as working reference code rather than part of the live build.
...
**Research (04)** researches the submitted technology from external sources restricted to a domain allowlist, with every claim carrying a citation. It slots in alongside the standards agent, and its output feeds the ADR author as additional context.
